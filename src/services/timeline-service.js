...
class TimelineService {
  ...
  async read({ date = "", limit = 0 } = {}) {
    const args = [];
    if (date) {
      args.push("--date", date);
    }
    const execution = await this.timelineIntegration.runSubcommand("read", args);
    const data = parseTimelineJsonOutput(execution, "read");
    // Trim the events array to keep the model-facing payload small.
    if (Number.isInteger(limit) && limit > 0 && Array.isArray(data?.events) && data.events.length > limit) {
