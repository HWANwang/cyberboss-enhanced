...
const { createHermesRuntimeAdapter } = require("../adapters/runtime/hermes");
...
const { ReminderAudit } = require("../services/reminder-audit");
const {
...
const WATCHDOG_FAILURE_LIMIT = 15;
...
function createRuntimeAdapter(config) {
  ...
  if (config.runtime === "hermes") {
    return createHermesRuntimeAdapter(config);
  }
...
class CyberbossApp {
  constructor(config) {
    ...
    this.reminderAudit = new ReminderAudit({ logFile: config.reminderAuditLogFile });
    this.turnGateStore = new TurnGateStore({ staleMs: config.turnGateStaleMs });
    this.pendingInboundByScope = new Map();
    this.pendingImageInboundByScope = new Map();
    this.turnBoundaryScopeKeys = new Set();
    this.systemMessageDispatcher = null;
    this.systemMessageFlushPromise = null;
    ...
    this.pendingCompactByThreadId = new Map();
    this.runtimeEventChain = Promise.resolve();
    this.runtimeCommandDispatching = false;
  ...
  async start() {
    ...
    if (typeof this.channelAdapter.notifyStart === "function") {
      await this.channelAdapter.notifyStart();
    }
    ...
    try {
      this.reminderQueue.reconcile();
    } catch (error) {
      console.error(`[cyberboss] reminder reconcile failed: ${error.message}`);
    }
    this.scheduleDailyReminderReconcile();
      ...
      while (!shutdown.stopped) {
        try {
          await Promise.all([
            ...
            this.flushPendingRuntimeCommand(),
          ...
          if (consecutiveFailures >= WATCHDOG_FAILURE_LIMIT) {
            console.error(
              `[cyberboss] watchdog: ${consecutiveFailures} consecutive poll failures, exiting for auto-restart`
            );
            shutdown.dispose();
            this.clearPendingImageInboundTimers();
            await this.closeLocationServer().catch(() => {});
  ...
  isTurnDispatchBlocked(bindingKey, workspaceRoot, { ignoreBoundary = false } = {}) {
    ...
    if (!ignoreBoundary && scopeKey && this.turnBoundaryScopeKeys?.has(scopeKey)) {
      console.warn(`[cyberboss] turn blocked by boundary scope=${scopeKey}`);
      return true;
    }
    const threadId = this.runtimeAdapter.getSessionStore().getThreadIdForWorkspace(bindingKey, workspaceRoot);
    const threadState = threadId ? this.threadStateStore.getThreadState(threadId) : null;
    const pendingGate = this.turnGateStore.getPending?.(bindingKey, workspaceRoot) || null;
    if (pendingGate) {
      const terminalStateMatches = Boolean(
        pendingGate.threadId
        && pendingGate.threadId === threadId
        && pendingGate.turnId
        && pendingGate.turnId === threadState?.turnId
        && (threadState?.status === "idle" || threadState?.status === "failed")
      );
      if (terminalStateMatches || pendingGate.stale) {
        this.turnGateStore.releaseScope(bindingKey, workspaceRoot);
        console.warn(
          `[cyberboss] released stale turnGate reason=${terminalStateMatches ? "terminal-thread-state" : "timeout"}`
          + ` thread=${pendingGate.threadId || threadId || "-"} turn=${pendingGate.turnId || "-"}`
          + ` ageMs=${pendingGate.ageMs}`
        );
      } else {
        console.warn(`[cyberboss] turn blocked by turnGate binding=${bindingKey} ws=${workspaceRoot}`);
        return true;
      }
    } else if (this.turnGateStore.isPending(bindingKey, workspaceRoot)) {
      console.warn(`[cyberboss] turn blocked by turnGate binding=${bindingKey} ws=${workspaceRoot}`);
      return true;
    }
    if (threadState?.status === "running" || hasRpcId(threadState?.pendingApproval?.requestId)) {
      console.warn(`[cyberboss] turn blocked by threadState status=${threadState?.status} thread=${threadId}`);
      return true;
    }
    return false;
  }
  ...
  async dispatchPreparedTurn({ bindingKey, workspaceRoot, prepared }) {
    ...
    try {
      ...
      console.log(`[cyberboss] dispatch turn start ws=${workspaceRoot} sender=${prepared.senderId.slice(0, 8)}`);
      ...
      console.log(`[cyberboss] dispatch turn done thread=${turn.threadId} in ${Date.now() - turnStartAt}ms`);
      ...
      this.turnGateStore.attachThread(pendingScopeKey, turn.threadId, turn.turnId);
      const replyTarget = {
        userId: prepared.senderId,
        contextToken: prepared.contextToken,
  ...
  async flushPendingSystemMessages() {
    if (this.systemMessageFlushPromise) {
      return this.systemMessageFlushPromise;
    }
    const running = this.flushPendingSystemMessagesOnce();
    this.systemMessageFlushPromise = running;
    try {
      await running;
    } finally {
      if (this.systemMessageFlushPromise === running) {
        this.systemMessageFlushPromise = null;
      }
    }
  }

  async flushPendingSystemMessagesOnce() {
    ...
    for (const message of pendingMessages) {
      try {
        ...
        if (!dispatched) {
          this.requeueBlockedSystemMessage(message);
        }
      } catch (error) {
        this.requeueBlockedSystemMessage(message);
        console.error(`[cyberboss] system event dispatch failed id=${message?.id || "-"}: ${error?.message || error}`);
      }
    }
  }

  requeueBlockedSystemMessage(message) {
    const attemptCount = Math.max(0, Number.parseInt(message?.attemptCount, 10) || 0);
    const delayMs = Math.min(60_000, 1_000 * (2 ** Math.min(attemptCount, 6)));
    this.systemMessageDispatcher?.requeue(message, { delayMs });
    console.warn(
      `[cyberboss] system event deferred id=${message?.id || "-"} attempt=${attemptCount + 1} delayMs=${delayMs}`
    );
  }
  ...
  async flushDueReminders(account) {
    const now = Date.now();
    const overdueSweep = this.reminderQueue.sweepOverdueScheduled(now, {
      graceMs: 24 * 60 * 60_000,
    });
    for (const expired of overdueSweep.expired) {
      this.reminderAudit.write({
        action: "expire_overdue",
        reminderId: expired.id,
        kind: expired.kind,
        dueAtMs: expired.dueAtMs,
        overdueMs: now - expired.dueAtMs,
      });
    }
    const dueReminders = overdueSweep.recoverable
      .filter((reminder) => reminder.accountId === account.accountId);

    for (const reminder of dueReminders) {
      try {
        if (reminder.kind === "todo_check") {
          const decision = this.resolveTodoCheckDecision(reminder);
          if (!decision.shouldFire) {
            this.reminderQueue.cancel(reminder.id);
            this.reminderAudit.write({
              action: "noop",
        ...
        this.reminderQueue.markFired(reminder.id);
        this.reminderAudit.write({
          action: "fire",
          reminderId: reminder.id,
          kind: reminder.kind,
    ...
    // Retry unacknowledged fixed_time reminders after a grace period.
    const RETRY_GRACE_MS = 10 * 60_000;
    const MAX_ATTEMPTS = 3;
    for (const fired of this.reminderQueue.listUnacknowledged(now)) {
      if (fired.accountId !== account.accountId) {
        continue;
      }
      const firedAt = fired.triggeredAt ? Date.parse(fired.triggeredAt) : fired.dueAtMs;
      if (!Number.isFinite(firedAt) || now - firedAt < RETRY_GRACE_MS) {
        continue;
      }
      const attempts = Number.isInteger(fired.attempts) ? fired.attempts : 0;
      if (fired.kind === "fixed_time" && attempts < MAX_ATTEMPTS) {
        this.reminderQueue.retry(fired.id, now + 1_000);
        this.reminderAudit.write({
          action: "retry",
          reminderId: fired.id,
          kind: fired.kind,
        ...
        this.reminderQueue.expire(fired.id);
        this.reminderAudit.write({
          action: "expire",
          reminderId: fired.id,
          kind: fired.kind,
  ...
  async flushPendingRuntimeCommand() {
    if (this.runtimeCommandDispatching || !this.config.runtimeCommandFile) {
      return;
    }
    const command = readJsonFile(this.config.runtimeCommandFile);
    if (!command || command.status !== "pending") {
      return;
    }
    this.runtimeCommandDispatching = true;
    try {
      if (command.type !== "compact_thread" || !normalizeText(command.threadId)) {
        writeJsonFile(this.config.runtimeCommandFile, {
          ...command,
          status: "failed",
      ...
      this.pendingCompactByThreadId.set(threadId, {
        kind: "compact",
        silent: true,
      ...
      await this.runtimeAdapter.compactThread({ threadId });
      console.log(`[cyberboss] compact command accepted thread=${threadId}`);
    } catch (error) {
      writeJsonFile(this.config.runtimeCommandFile, {
        ...command,
        status: "failed",
  ...
  /**
   * todo_check reminders only fire if the referenced todo is still the same
   * plan (same version) and still unfinished. Stale reminders are no-ops.
   */
  resolveTodoCheckDecision(reminder) {
    const todo = this.projectServices?.todo;
    if (!todo) {
      return { shouldFire: false, reason: "todo service unavailable" };
    }
    const item = todo.getById(String(reminder.sourceTodoId || "").trim());
    if (!item) {
      return { shouldFire: false, reason: "todo not found" };
    }
    if (item.status === "completed") {
      return { shouldFire: false, reason: "todo completed" };
    }
    const reminderVersion = Number(reminder.todoVersion) || 0;
    const todoVersion = Number(item.version) || 0;
    if (reminderVersion > 0 && todoVersion > 0 && reminderVersion !== todoVersion) {
      return { shouldFire: false, reason: "todo plan changed (version)" };
    }
    return { shouldFire: true, reason: "" };
  }

  scheduleDailyReminderReconcile() {
    const runAt = nextDailyReconcileTime();
    const delay = Math.max(60_000, runAt - Date.now());
    setTimeout(() => {
      try {
        this.reminderQueue.reconcile();
      } catch (error) {
  ...
  async dispatchSystemMessage(message) {
    ...
    console.log(
      `[cyberboss] system event dispatch id=${message.id} account=${prepared.accountId} user=${prepared.senderId} ws=${workspaceRoot}`
    );
    if (this.isTurnDispatchBlocked(bindingKey, workspaceRoot)) {
      return false;
    }
  ...
  async handleCompactCommand(normalized) {
    ...
    try {
      ...
      if (!this.pendingCompactByThreadId) {
        this.pendingCompactByThreadId = new Map();
      }
      if (!trackedByTurn) {
        this.pendingCompactByThreadId.set(threadId, {
  ...
  async handleModelCommand(normalized, command) {
    ...
    const runtimeParams = sessionStore.getRuntimeParamsForWorkspace(bindingKey, workspaceRoot);
    const currentModel = normalizeText(runtimeParams?.model)
      || normalizeText(this.config.codexModel)
      || "(default)";

    const modelInput = normalizeCommandArgument(command.args);
    if (!modelInput) {
    ...
    const requested = String(modelInput).trim().toLowerCase();
    const match = models.find((item) => String(item.id || "").trim().toLowerCase() === requested);
    if (!match) {
      await this.channelAdapter.sendText({
        userId: normalized.senderId,
        text: `⚠️  Model not found: ${modelInput}. Use /model to list available models.`,
        contextToken: normalized.contextToken,
      });
      return;
    }
    ...
    sessionStore.setRuntimeParamsForWorkspace(bindingKey, workspaceRoot, {
      model: match.id,
      modelProvider: normalizeText(this.config.codexModelProvider),
    });
    await this.channelAdapter.sendText({
      userId: normalized.senderId,
      text: `✅ Model switched to ${match.id}.\nApplies from the next message (same conversation thread is kept).\nDefault (.env): ${this.config.codexModel}`,
      contextToken: normalized.contextToken,
    });
  }
  ...
  async handleRuntimeEvent(event) {
    const failureReplyTarget = event?.type === "runtime.turn.failed"
      ...
      : null;
    const isTerminalTurnEvent = event?.type === "runtime.turn.completed" || event?.type === "runtime.turn.failed";
    try {
    ...
    if (event.type === "runtime.context.compacted") {
      const compactThreadId = normalizeText(event.payload.threadId)
        || (this.pendingCompactByThreadId?.size === 1
          ? this.pendingCompactByThreadId.keys().next().value
          : "");
      const pending = this.pendingCompactByThreadId?.get?.(compactThreadId) || null;
      this.pendingCompactByThreadId?.delete?.(compactThreadId);
      if (pending?.silent && this.config.runtimeCommandFile) {
        const command = readJsonFile(this.config.runtimeCommandFile) || {};
        writeJsonFile(this.config.runtimeCommandFile, {
          ...command,
          status: "completed",
        ...
        console.log(`[cyberboss] compact command completed thread=${compactThreadId}`);
      } else if (pending) {
        await this.channelAdapter.sendText({
          userId: pending.userId,
          text: `✅ Compact finished\nthread: ${compactThreadId}`,
...
function buildReminderSystemTrigger(reminder, config = {}) {
  return JSON.stringify({
    ...
    kind: String(reminder?.kind || "fixed_time").trim(),
    text: String(reminder?.text || "").trim(),
    sourceTodoId: String(reminder?.sourceTodoId || "").trim(),
  });
}

function nextDailyReconcileTime() {
  const now = new Date();
  const next = new Date(now);
  next.setHours(0, 10, 0, 0); // 00:10 local
