...
class StreamDelivery {
  constructor({ channelAdapter, sessionStore, runtimeId = "", onDeferredSystemReply, systemReplyRetryScheduleMs }) {
  ...
  setReplyTarget(bindingKey, target) {
    ...
    this.replyTargetByBindingKey.set(bindingKey, {
      ...
      messageId: normalizeText(target.messageId),
    });
  }
  ...
  async handleRuntimeEvent(event) {
    ...
    switch (event.type) {
      ...
      case "runtime.reply.discarded": {
        const state = this.ensureRunState(threadId, turnId);
        this.discardItem(state, normalizeText(event.payload.itemId));
        return;
      }
  ...
  async flushNow(state, { force }) {
    ...
    // A WeChat context token is consumed by outbound API calls, not by visible
    // bubbles.  Sending every completed runtime item immediately can therefore
    // exhaust a token before the channel adapter gets a chance to pack chunks.
    // Keep ordinary WeChat replies in this turn until the final flush, then
    // send their plain-text items as one delivery.
    if (state.replyTarget.provider === "weixin" && !force) {
      return;
    }

    const pendingDeliveries = coalesceWeixinPlainReplyDeliveries(
      collectPendingReplyDeliveries(state, { force }),
      state.replyTarget.provider === "weixin"
    );
    ...
    state.sendChain = state.sendChain.then(async () => {
      for (let index = 0; index < pendingDeliveries.length; index += 1) {
        ...
        for (const itemId of delivery.itemIds || [delivery.itemId]) {
          state.sentItemIds.add(itemId);
        }
  ...
  async sendReplyDelivery(state, delivery, { prependDeferredPrefix = false } = {}) {
    ...
    if (this.channelAdapter.supportsIdempotentDelivery) {
      payload.deliveryId = `${state.runKey}:${delivery.itemId}:plain_reply`;
    }
  ...
  async sendSystemReply(state, text) {
    ...
    if (this.channelAdapter.supportsIdempotentDelivery) {
      payload.deliveryId = `${state.runKey}:system_reply`;
    }
  ...
  async sendTextWithRetry(state, payload, { kind }) {
    const initialTarget = state.replyTarget;
    try {
      const receipts = await this.channelAdapter.sendText(payload);
      console.log(
        `[cyberboss] WeChat outbound accepted thread=${state.threadId} event=${initialTarget?.messageId || "-"} user=${payload.userId} receipts=${formatDeliveryReceipts(receipts)}`
      );
      return;
    } catch (error) {
      ...
      try {
        ...
        if (payload.deliveryId) {
          retryPayload.deliveryId = payload.deliveryId;
        }
        if (payload.preserveBlock) {
          retryPayload.preserveBlock = true;
        }
        const receipts = await this.channelAdapter.sendText(retryPayload);
        ...
        console.log(
          `[cyberboss] WeChat outbound accepted after retry thread=${state.threadId} event=${initialTarget?.messageId || "-"} user=${retryPayload.userId} receipts=${formatDeliveryReceipts(receipts)}`
        );
      } catch (retryError) {
...
function coalesceWeixinPlainReplyDeliveries(deliveries, enabled) {
  if (!enabled) {
    return deliveries;
  }
  ...
  for (const delivery of deliveries) {
    ...
    if (!plainBuffer) {
      plainBuffer = { ...delivery, itemIds: [delivery.itemId] };
      continue;
    }
    plainBuffer.text = `${plainBuffer.text}\n\n${delivery.text}`;
    plainBuffer.itemIds.push(delivery.itemId);
  }
  flushPlainBuffer();
  return coalesced;
}
...
function formatDeliveryReceipts(receipts) {
  const list = Array.isArray(receipts) ? receipts : receipts ? [receipts] : [];
  if (!list.length) {
    return "[]";
  }
  return JSON.stringify(list.map((receipt) => ({
    messageId: normalizeText(receipt?.message_id || receipt?.msg_id),
    ret: receipt?.ret ?? null,
    errcode: receipt?.errcode ?? null,
  })));
}
...
function resolveSystemReplyAction(candidate) {
  const parsed = tryParseJson(candidate) || tryParseJson(extractLeadingJsonObject(candidate));
...
function extractLeadingJsonObject(candidate) {
