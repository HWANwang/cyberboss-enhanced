class ThreadStateStore {
  ...
  applyRuntimeEvent(event) {
    if (event?.type === "runtime.context.updated") {
      ...
      // Observability: log token usage so cost growth can be inspected.
      try {
        const u = event?.payload || {};
        console.log(
          `[cyberboss] usage thread=${u.threadId || "-"} in=${u.inputTokens} cached=${u.cachedInputTokens} out=${u.outputTokens} reasoning=${u.reasoningTokens} total=${u.currentTokens}`,
        );
      } catch {
        // never break the pipeline over logging
      }
      if (runtimeId) {
