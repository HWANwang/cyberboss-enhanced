...
function readConfig() {
  ...
  const privateDir = readPathEnv("CYBERBOSS_PRIVATE_DIR") || path.join(stateDir, "private");
  ...
  return {
    ...
    reminderAuditLogFile: path.join(stateDir, "logs", "reminder-audit.jsonl"),
    ...
    runtimeCommandFile: path.join(stateDir, "runtime-command.json"),
    weixinInstructionsFile: readPathEnv("CYBERBOSS_WEIXIN_INSTRUCTIONS_FILE") || path.join(privateDir, "weixin-instructions.md"),
    weixinOperationsFile: readPathEnv("CYBERBOSS_WEIXIN_OPERATIONS_FILE") || path.join(privateDir, "weixin-operations.md"),
    ...
    turnGateStaleMs: Math.max(60_000, readIntEnv("CYBERBOSS_TURN_GATE_STALE_MS") || 15 * 60_000),
