...
const { getConfig, sendTyping, notifyStart } = require("./api");
...
function createWeixinChannelAdapter(config) {
  ...
  async function sendTextChunks({ userId, text, contextToken = "", preserveBlock = false, deliveryId = "" }) {
    const account = ensureAccount();
    ...
    for (let index = 0; index < sendChunks.length; index += 1) {
      const deliveryChunk = finalizeWeixinDeliveryChunk(sendChunks[index]) || "Completed.";
      receipts.push(await sendText({
        baseUrl: account.baseUrl,
        token: account.token,
        toUserId: userId,
        text: deliveryChunk,
        contextToken: resolvedToken,
        clientId: buildDeliveryClientId(deliveryId, index, deliveryChunk),
      }));
  ...
  return {
    ...
    async notifyStart() {
      const account = ensureAccount();
      return notifyStart({ baseUrl: account.baseUrl, token: account.token });
    },
    ...
    async sendText({ userId, text, contextToken = "", preserveBlock = false, deliveryId = "" }) {
      return sendTextChunks({ userId, text, contextToken, preserveBlock, deliveryId });
    },
...
function buildDeliveryClientId(deliveryId, chunkIndex, text) {
