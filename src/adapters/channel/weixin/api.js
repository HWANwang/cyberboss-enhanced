...
// Keep the wire protocol version aligned with Tencent's current iLink plugin.
// This is independent from Cyberboss's own package version.
...
function buildBaseInfo() {
  return {
    ...
    bot_agent: `Cyberboss/${CHANNEL_VERSION}`,
  };
}
...
async function notifyStart({ baseUrl, token, timeoutMs }) {
  const raw = await apiPost({
    baseUrl,
  ...
  return parseJson(raw, "notifyStart");
}
