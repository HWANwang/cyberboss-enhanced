...
function buildCdnUploadUrl({ cdnBaseUrl, uploadFullUrl, uploadParam, filekey }) {
  const fullUrl = typeof uploadFullUrl === "string" ? uploadFullUrl.trim() : "";
  if (fullUrl) {
    return fullUrl;
  }
...
async function uploadMediaToWeixin({ filePath, toUserId, opts, cdnBaseUrl, mediaType }) {
  ...
  const uploadFullUrl = typeof uploadUrlResp?.upload_full_url === "string"
    ...
    : "";
  const uploadParam = typeof uploadUrlResp?.upload_param === "string"
    ? uploadUrlResp.upload_param.trim()
    : "";
...
module.exports = { buildCdnUploadUrl, sendWeixinMediaFile };
