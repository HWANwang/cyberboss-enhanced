...
function getThreadKeyRuntimeMap(binding) {
  return binding?.threadIdByThreadKeyByWorkspaceRootByRuntime
    && typeof binding.threadIdByThreadKeyByWorkspaceRootByRuntime === "object"
    ? binding.threadIdByThreadKeyByWorkspaceRootByRuntime
    : {};
}

function getThreadKeyMapForRuntime(binding, runtimeId) {
  const normalizedRuntimeId = normalizeValue(runtimeId);
  const runtimeMap = getThreadKeyRuntimeMap(binding);
  if (!normalizedRuntimeId) {
    return {};
  }
  const scoped = runtimeMap[normalizedRuntimeId];
