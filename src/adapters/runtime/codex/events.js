...
function mapCodexMessageToRuntimeEvent(message) {
  ...
  if (message?.type === "event_msg" && message?.payload?.type === "context_compacted") {
    return {
      type: "runtime.context.compacted",
      payload: {
        threadId: normalizeString(message?.thread_id || message?.payload?.thread_id),
        turnId: normalizeString(message?.payload?.turn_id),
      },
    };
  }
  ...
  if (method === "item/agentMessage/delta") {
    if (isCommentaryAgentMessage(params)) {
      return null;
    }
...
function isCommentaryAgentMessage(params) {
  const phase = normalizeString(params?.phase || params?.item?.phase).toLowerCase();
