...
class RuntimeContextStore {
  ...
  resolveActiveContext({ workspaceRoot = "", runtimeId = "" } = {}) {
    // The MCP server can outlive account and thread switches. Reload the
    // persisted context for every tool call instead of using startup state.
    this.load({ preserveOnError: true });
    const normalizedWorkspaceRoot = normalizeText(workspaceRoot);
    if (normalizedWorkspaceRoot) {
