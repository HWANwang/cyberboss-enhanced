...
function spawnDetachedCommand(command, args, { logFile, cwd = rootDir, env = {} } = {}) {
  ...
  const child = spawn(command, args, {
    ...
    // Passing an argument array through cmd.exe on Windows loses the quoting
    // needed by Codex's repeated -c TOML overrides. Spawn the executable
    // directly so the app-server receives its MCP configuration intact.
    shell: false,
  });
  child.unref();
  return child.pid;
...
async function ensureSharedAppServer() {
  ...
  const command = resolveCodexSpawnCommand(process.env.CYBERBOSS_CODEX_COMMAND || "codex");
...
function resolveCodexSpawnCommand(command) {
  const configured = normalizeText(command) || "codex";
  if (process.platform !== "win32" || !/\.cmd$/i.test(configured)) {
    return configured;
  }

  // The managed Windows wrapper keeps a stable Codex executable beside state.
  // Spawn that executable directly: .cmd wrappers require cmd.exe and would
  // reintroduce lossy quoting for the -c MCP configuration overrides above.
  const stableExecutable = path.join(stateDir, "codex-runtime", "codex.exe");
  return fs.existsSync(stableExecutable) ? stableExecutable : configured;
}
