...
test("delivery client ids are stable for idempotent retries and unique per chunk", () => {
  const first = buildDeliveryClientId("thread-1:turn-1:item-1", 0, "正文");
  assert.equal(first, buildDeliveryClientId("thread-1:turn-1:item-1", 0, "正文"));
  assert.notEqual(first, buildDeliveryClientId("thread-1:turn-1:item-1", 1, "正文"));
  assert.match(first, /^cb-[a-f0-9]{32}$/);
});
