...
test("claudecode approval events canonicalize diary commands for stable always matching", () => {
  const event = mapClaudeCodeMessageToRuntimeEvent({
    ...
    toolName: "exec_command",
    input: {
      cmd: "/workspace/cyberboss/bin/cyberboss diary write --date 2026-04-17 --title '4.17' --text 'hello'",
    },
  });
...
test("claudecode approval events canonicalize Read image approvals for stable matching", () => {
  const event = mapClaudeCodeMessageToRuntimeEvent({
    ...
    toolName: "Read",
    input: {
      file_path: "/tmp/cyberboss-state/inbox/2026-04-17/attachment-5.jpg",
    },
  });

  assert.deepEqual(event.payload.commandTokens, ["read_image"]);
  assert.equal(event.payload.filePath, "/tmp/cyberboss-state/inbox/2026-04-17/attachment-5.jpg");
});
...
test("claudecode approval events keep non-image Read approvals as file reads", () => {
  const event = mapClaudeCodeMessageToRuntimeEvent({
    ...
    toolName: "Read",
    input: {
      file_path: "/tmp/cyberboss-state/inbox/2026-04-17/note.txt",
    },
  });

  assert.deepEqual(event.payload.commandTokens, []);
  assert.equal(event.payload.filePath, "/tmp/cyberboss-state/inbox/2026-04-17/note.txt");
});
...
test("claudecode approval events capture Write file paths for state-dir auto approve", () => {
  const event = mapClaudeCodeMessageToRuntimeEvent({
    ...
    toolName: "Write",
    input: {
      file_path: "/tmp/cyberboss-state/notes/today.md",
      content: "hello",
    },
  });

  assert.equal(event.payload.filePath, "/tmp/cyberboss-state/notes/today.md");
  assert.deepEqual(event.payload.filePaths, ["/tmp/cyberboss-state/notes/today.md"]);
});
...
test("handleCompactCommand tracks Codex compaction by thread when no turn id is returned", async () => {
  const appLike = {
    pendingOperationByRunKey: new Map(),
    pendingCompactByThreadId: new Map(),
    resolveWorkspaceRoot() { return "/workspace"; },
    runtimeAdapter: {
      async compactThread() { return {}; },
      getSessionStore() {
        return {
          buildBindingKey() { return "binding-1"; },
          getThreadIdForWorkspace() { return "thread-1"; },
          getRuntimeParamsForWorkspace() { return { model: "gpt-5.6-sol" }; },
        };
      },
    },
  ...
  assert.equal(appLike.pendingCompactByThreadId.get("thread-1")?.userId, "user-1");
});
...
test("handleRuntimeEvent reports Codex thread compaction completion", async () => {
  const sent = [];
  const appLike = {
    pendingCompactByThreadId: new Map([["thread-1", { userId: "user-1", contextToken: "ctx-1" }]]),
  ...
  assert.deepEqual(sent, ["✅ Compact finished\nthread: thread-1"]);
  assert.equal(appLike.pendingCompactByThreadId.size, 0);
});
