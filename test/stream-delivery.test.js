...
function createHarness({ sendText, getKnownContextTokens, runtimeId = "", supportsIdempotentDelivery = false } = {}) {
  const sent = [];
  const channelAdapter = {
    async sendText(payload) {
      if (typeof sendText === "function") {
        return await sendText(payload, sent);
      }
      sent.push(payload);
    },
    supportsIdempotentDelivery,
...
test("empty successful WeChat receipts are accepted without retry", async () => {
  const attempts = [];
  ...
  streamDelivery.queueReplyTargetForThread("thread-ambiguous", {
    userId: "user-ambiguous",
    contextToken: "ctx-ambiguous",
    provider: "weixin",
  ...
  assert.match(attempts[0].deliveryId, /thread-ambiguous:turn-ambiguous/);
});
...
test("system send_message tolerates a trailing ellipsis artifact", async () => {
  const { sent, streamDelivery } = createHarness({ runtimeId: "codex" });
  streamDelivery.queueReplyTargetForThread("thread-2e", {
    userId: "user-2e",
    contextToken: "ctx-2e",
    provider: "system",
  });
  ...
  await runCompletedTurn(streamDelivery, {
    ...
    turnId: "turn-2e",
    itemId: "item-2e",
    text: "{\"action\":\"send_message\",\"message\":\"提醒时间到了\"}  ...",
  });
...
test("system send_message ignores trailing protocol contamination", async () => {
  const { sent, streamDelivery } = createHarness({ runtimeId: "codex" });
  streamDelivery.queueReplyTargetForThread("thread-2x", {
    userId: "user-2x",
    contextToken: "ctx-2x",
    provider: "system",
  });
  ...
  await runCompletedTurn(streamDelivery, {
    ...
    turnId: "turn-2x",
    itemId: "item-2x",
    text: "{\"action\":\"send_message\",\"message\":\"提醒正文\"} ...\n</conversation> internal commentary",
  });
...
test("system send_message rejects commentary before the JSON action", async () => {
  const { sent, streamDelivery } = createHarness({ runtimeId: "codex" });
  ...
  await runCompletedTurn(streamDelivery, {
    ...
    turnId: "turn-2p",
    itemId: "item-2p",
    text: "先解释一下。\n{\"action\":\"send_message\",\"message\":\"不应发送\"}",
  });
...
test("plain WeChat reply with deferred prefix waits for turn completion", async () => {
  const { sent, streamDelivery, bindingByThreadId } = createHarness();
...
test("plain WeChat reply items are combined into one outbound delivery", async () => {
  const { sent, streamDelivery } = createHarness();
  streamDelivery.queueReplyTargetForThread("thread-9", {
    userId: "user-9",
    contextToken: "ctx-9",
    provider: "weixin",
  });
  ...
  await streamDelivery.handleRuntimeEvent({
    type: "runtime.turn.started",
    payload: { threadId: "thread-9", turnId: "turn-9" },
  });
  for (const [itemId, text] of [["item-9a", "第一段"], ["item-9b", "第二段"], ["item-9c", "第三段"]]) {
    await streamDelivery.handleRuntimeEvent({
      type: "runtime.reply.completed",
      payload: { threadId: "thread-9", turnId: "turn-9", itemId, text },
    });
  }
