...
test("turn gate ignores a late completion from an older turn on the same thread", () => {
  const gate = new TurnGateStore();
  const firstScope = gate.begin("binding-1", "/workspace");
  gate.attachThread(firstScope, "thread-1", "turn-1");
  gate.releaseThread("thread-1", "turn-1");

  const secondScope = gate.begin("binding-1", "/workspace");
  gate.attachThread(secondScope, "thread-1", "turn-2");

  assert.equal(gate.releaseThread("thread-1", "turn-1"), false);
  assert.equal(gate.isPending("binding-1", "/workspace"), true);
  assert.equal(gate.releaseThread("thread-1", "turn-2"), true);
  assert.equal(gate.isPending("binding-1", "/workspace"), false);
});

test("turn gate exposes expired pending state for watchdog recovery", () => {
  let now = 1_000;
  const gate = new TurnGateStore({ staleMs: 5_000, now: () => now });
  const scope = gate.begin("binding-1", "/workspace");
  gate.attachThread(scope, "thread-1", "turn-1");

  now = 7_000;
  const pending = gate.getPending("binding-1", "/workspace");
  assert.equal(pending.stale, true);
  assert.equal(pending.ageMs, 6_000);
});

test("dispatch blocking reconciles a gate whose exact turn is already terminal", () => {
  const gate = new TurnGateStore();
  const scope = gate.begin("binding-1", "/workspace");
  gate.attachThread(scope, "thread-1", "turn-1");
  const appLike = {
    ...
    threadStateStore: {
      getThreadState() {
        return { status: "idle", turnId: "turn-1", pendingApproval: null };
      },
    },
  };

  assert.equal(CyberbossApp.prototype.isTurnDispatchBlocked.call(appLike, "binding-1", "/workspace"), false);
  assert.equal(gate.isPending("binding-1", "/workspace"), false);
});

test("terminal runtime events release the turn gate even when reply delivery throws", async () => {
  const releases = [];
  const appLike = {
    streamDelivery: {
    ...
    turnGateStore: {
      releaseThread(threadId, turnId) {
        releases.push({ threadId, turnId });
      },
    },
  };

  await assert.rejects(
    CyberbossApp.prototype.handleRuntimeEvent.call(appLike, {
      type: "runtime.turn.completed",
      payload: { threadId: "thread-1", turnId: "turn-1" },
    }),
    /delivery failed/,
  );
  assert.deepEqual(releases, [{ threadId: "thread-1", turnId: "turn-1" }]);
});
