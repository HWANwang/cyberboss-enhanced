...
下面命令使用 shell 变量 `PROJECT_DIR` 指向项目目录：

```bash
PROJECT_DIR=/absolute/path/to/cyberboss
```
...
```bash
tmux new-session -d -s cb-bridge -c $PROJECT_DIR 'npm run shared:start; exec zsh'
```
...
```bash
tmux new-session -d -s cb-open -c $PROJECT_DIR 'npm run shared:open; exec zsh'
```
...
```bash
tmux new-session -d -s cb-open -c $PROJECT_DIR 'CYBERBOSS_RUNTIME=codex npm run shared:open; exec zsh'
```
